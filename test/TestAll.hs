...
prop_minimumSequence ints = monadicIO $ do
  ...
  minSeq <- run $ Sequence.getMinimumSequence seqs
  if null ints
...
sequenceTests =
  [ quickCheck prop_deterministic_setGetSequence
  , quickCheck prop_deterministic_addAndGetSequence
  , quickCheck prop_setVolatileGetSequence
  , quickCheck prop_compareAndSetSequence
  , quickCheck prop_sameSequenceIsSame
  , quickCheck prop_differentSequenceIsNotSame
  , quickCheck prop_minimumSequence
  ]

main = do
  sequence_ sequenceTests
